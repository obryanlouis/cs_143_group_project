...
int main(int argc, char **argv) {
    ...
    for (list<FlowInfo>::iterator it = flows.begin(); it != flows.end(); it++)
    {
        Flow *f = new Flow(it->flowId, it->flowSize, hostsById[it->sourceId],
                hostsById[it->destinationId]);
    ...
    {
        Node *n1 = getNode(it->node1Type, it->node1Id, hostsById, routersById);
        Node *n2 = getNode(it->node2Type, it->node2Id, hostsById, routersById);
